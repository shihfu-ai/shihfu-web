...
// Keyed by the parent field's value (or, for multi-parent fields, by
// each parent's value joined with "::" in the same order as
// `dependsOn`). Used by fields that set `dependsOn` + `optionsMap`.

const BREEDS_BY_PET_TYPE = {
  Dog: ['Labrador Retriever', 'Golden Retriever', 'German Shepherd', 'Beagle', 'Pomeranian', 'Shih Tzu', 'Pug', 'Rottweiler', 'Dachshund', 'Boxer', 'Doberman', 'Cocker Spaniel', 'Great Dane', 'Siberian Husky', 'Indian Pariah / Indie', 'Other'],
  Cat: ['Persian', 'Siamese', 'Maine Coon', 'British Shorthair', 'Bengal', 'Ragdoll', 'Indie / Mixed', 'Other'],
  Bird: ['Budgerigar (Budgie)', 'Cockatiel', 'Lovebird', 'African Grey Parrot', 'Macaw', 'Pigeon', 'Other'],
  Rabbit: ['Dutch', 'Angora', 'Lionhead', 'Netherland Dwarf', 'Mixed', 'Other'],
  'Guinea Pig': ['American', 'Abyssinian', 'Peruvian', 'Silkie', 'Other'],
  Fish: ['Goldfish', 'Betta', 'Guppy', 'Angelfish', 'Koi', 'Other'],
  Reptile: ['Turtle', 'Tortoise', 'Lizard', 'Iguana', 'Snake', 'Other'],
  Other: ['Other'],
};

const VEHICLE_MAKES_BY_TYPE = {
  Car: ['Maruti Suzuki', 'Hyundai', 'Tata', 'Mahindra', 'Honda', 'Toyota', 'Kia', 'Volkswagen', 'Skoda', 'Renault', 'Nissan', 'Ford', 'MG', 'Jeep', 'BMW', 'Mercedes-Benz', 'Audi', 'Other'],
  'Two-Wheeler': ['Hero', 'Honda', 'TVS', 'Bajaj', 'Royal Enfield', 'Yamaha', 'Suzuki', 'KTM', 'Ather', 'Ola Electric', 'Other'],
  'Commercial Vehicle': ['Tata', 'Ashok Leyland', 'Mahindra', 'Eicher', 'Force Motors', 'Other'],
};

// Keyed by "Vehicle Type::Make" since the same brand name (e.g. Honda,
// Tata) sells completely different model lines for cars vs two-wheelers
// vs commercial vehicles.
const VEHICLE_MODELS_BY_TYPE_AND_MAKE = {
  'Car::Maruti Suzuki': ['Swift', 'Baleno', 'WagonR', 'Alto', 'Dzire', 'Ertiga', 'Brezza', 'Ciaz', 'Celerio', 'XL6', 'Fronx', 'Other'],
  'Car::Hyundai': ['i10', 'i20', 'Venue', 'Creta', 'Verna', 'Aura', 'Exter', 'Alcazar', 'Tucson', 'Other'],
  'Car::Tata': ['Nexon', 'Punch', 'Tiago', 'Tigor', 'Altroz', 'Harrier', 'Safari', 'Curvv', 'Other'],
  'Car::Mahindra': ['Scorpio', 'XUV700', 'XUV300', 'Thar', 'Bolero', 'XUV400', 'Other'],
  'Car::Honda': ['City', 'Amaze', 'Elevate', 'Jazz', 'WR-V', 'Other'],
  'Car::Toyota': ['Innova Crysta', 'Fortuner', 'Glanza', 'Urban Cruiser', 'Camry', 'Innova Hycross', 'Other'],
  'Car::Kia': ['Seltos', 'Sonet', 'Carens', 'Carnival', 'EV6', 'Other'],
  'Car::Volkswagen': ['Polo', 'Virtus', 'Taigun', 'Tiguan', 'Other'],
  'Car::Skoda': ['Kushaq', 'Slavia', 'Kodiaq', 'Superb', 'Other'],
  'Car::Renault': ['Kwid', 'Triber', 'Kiger', 'Duster', 'Other'],
  'Car::Nissan': ['Magnite', 'Kicks', 'Other'],
  'Car::Ford': ['EcoSport', 'Endeavour', 'Figo', 'Other'],
  'Car::MG': ['Hector', 'Astor', 'ZS EV', 'Comet EV', 'Other'],
  'Car::Jeep': ['Compass', 'Meridian', 'Wrangler', 'Other'],
  'Car::BMW': ['3 Series', '5 Series', 'X1', 'X5', 'Other'],
  'Car::Mercedes-Benz': ['C-Class', 'E-Class', 'GLA', 'GLC', 'Other'],
  'Car::Audi': ['A4', 'A6', 'Q3', 'Q5', 'Other'],
  'Car::Other': ['Other'],

  'Two-Wheeler::Hero': ['Splendor', 'HF Deluxe', 'Passion', 'Glamour', 'Xtreme', 'Destini', 'Other'],
  'Two-Wheeler::Honda': ['Activa', 'Shine', 'Unicorn', 'SP125', 'Dio', 'Hornet', 'Other'],
  'Two-Wheeler::TVS': ['Jupiter', 'Apache', 'Ntorq', 'Raider', 'Sport', 'Other'],
  'Two-Wheeler::Bajaj': ['Pulsar', 'Platina', 'CT100', 'Avenger', 'Dominar', 'Other'],
  'Two-Wheeler::Royal Enfield': ['Classic 350', 'Bullet 350', 'Hunter 350', 'Meteor 350', 'Himalayan', 'Other'],
  'Two-Wheeler::Yamaha': ['FZ', 'R15', 'MT-15', 'Fascino', 'RayZR', 'Other'],
  'Two-Wheeler::Suzuki': ['Access 125', 'Burgman', 'Gixxer', 'Avenis', 'Other'],
  'Two-Wheeler::KTM': ['Duke 200', 'Duke 390', 'RC 200', 'Other'],
  'Two-Wheeler::Ather': ['450X', '450S', 'Rizta', 'Other'],
  'Two-Wheeler::Ola Electric': ['S1 Pro', 'S1 Air', 'S1 X', 'Other'],
  'Two-Wheeler::Other': ['Other'],

  'Commercial Vehicle::Tata': ['Ace', 'Intra', '407', 'LPT 1613', 'Winger', 'Other'],
  'Commercial Vehicle::Ashok Leyland': ['Dost', 'Bada Dost', 'Partner', 'Ecomet', 'Other'],
  'Commercial Vehicle::Mahindra': ['Bolero Pickup', 'Jeeto', 'Supro', 'Furio', 'Other'],
  'Commercial Vehicle::Eicher': ['Pro 2049', 'Pro 3015', 'Skyline Pro', 'Other'],
  'Commercial Vehicle::Force Motors': ['Traveller', 'Trump', 'Other'],
  'Commercial Vehicle::Other': ['Other'],
};

function recentYears(count = 30) {
  const current = new Date().getFullYear();
  return Array.from({ length: count }, (_, i) => String(current + 1 - i));
}

const PROPERTY_SIZE_BY_TYPE_CLEANING = {
  Apartment: ['Studio', '1 BHK', '2 BHK', '3 BHK', '4 BHK', 'Custom'],
  'Independent House': ['2 BHK', '3 BHK', '4 BHK', 'Custom'],
  Villa: ['3 BHK', '4 BHK', '5+ BHK', 'Custom'],
  Office: ['Office (Small)', 'Office (Large)', 'Custom'],
  'Commercial Space': ['Office (Small)', 'Office (Large)', 'Custom'],
  Other: ['Custom'],
};

const PROPERTY_SIZE_BY_TYPE_PEST = {
  'Residential - Apartment': ['1 BHK', '2 BHK', '3 BHK', '4 BHK', 'Custom'],
  'Residential - Independent House': ['2 BHK', '3 BHK', '4 BHK', 'Custom'],
  'Residential - Villa': ['Villa', 'Custom'],
  'Commercial - Office': ['Small Office', 'Large Office', 'Custom'],
  'Commercial - Restaurant': ['Small Office', 'Large Office', 'Custom'],
  'Commercial - Warehouse': ['Custom'],
  Industrial: ['Custom'],
  Other: ['Custom'],
};

const AC_MODELS_BY_BRAND = {
  LG: ['Dual Cool Series', 'Dual Inverter Series', 'Convertible 6-in-1 Series', 'Other'],
  Daikin: ['FTKF Series', 'FTKM Series', 'Inverter Split Series', 'Other'],
  Voltas: ['Adjustable Inverter Series', 'Vertis Series', 'All Star Series', 'Other'],
  'Blue Star': ['IC Series', 'ID Series', 'Inverter Splitaire Series', 'Other'],
  Carrier: ['Ester Series', 'Emperia Series', 'Midea Collaboration Series', 'Other'],
  Hitachi: ['Kaze Series', 'Zunoh Series', 'Merai Series', 'Other'],
  Samsung: ['WindFree Series', 'AR Series', 'Digital Inverter Series', 'Other'],
  Panasonic: ['Deluxe Series', 'Ziya Series', 'Prime Series', 'Other'],
  Godrej: ['NXW Series', 'EON Series', 'Other'],
  'O General': ['ASGA Series', 'ASGG Series', 'Other'],
  Other: ['Other'],
};

const PROPERTY_TYPE_BY_REQUIREMENT = {
  Buy: ['1 BHK', '2 BHK', '3 BHK', '4 BHK', 'Villa', 'Plot', 'Any'],
  Rent: ['1 BHK', '2 BHK', '3 BHK', '4 BHK', 'Villa', 'Any'],
  Invest: ['1 BHK', '2 BHK', '3 BHK', '4 BHK', 'Villa', 'Plot', 'Any'],
  Sell: ['1 BHK', '2 BHK', '3 BHK', '4 BHK', 'Villa', 'Plot', 'Any'],
  'Commercial - Buy': ['Commercial Office', 'Commercial Retail', 'Any'],
  'Commercial - Rent': ['Commercial Office', 'Commercial Retail', 'Any'],
};
...
export const INDUSTRY_CONFIG = {
  ...
  auto_repair: {
    ...
    assetFields: [
      ...
      { key: 'vehicleType',      label: 'Vehicle Type',                 type: 'select', options: ['Car', 'Two-Wheeler', 'Commercial Vehicle'] },
      { key: 'make',             label: 'Make',                         type: 'select', dependsOn: 'vehicleType', dependsOnLabel: 'Vehicle Type', optionsMap: VEHICLE_MAKES_BY_TYPE },
      { key: 'model',            label: 'Model',                        type: 'select', dependsOn: ['vehicleType','make'], dependsOnLabel: 'Make', optionsMap: VEHICLE_MODELS_BY_TYPE_AND_MAKE },
      { key: 'year',             label: 'Year of Manufacture',          type: 'select', options: recentYears() },
      { key: 'trim',             label: 'Trim / Variant',               type: 'select', options: ['Base', 'Mid', 'Top', 'Sport', 'Limited', 'Other'] },
  ...
  veterinary: {
    ...
    assetFields: [
      ...
      { key: 'breed',         label: 'Breed',                      type: 'select', dependsOn: 'petType', dependsOnLabel: 'Pet Type', optionsMap: BREEDS_BY_PET_TYPE },
  ...
  home_cleaning: {
    ...
    assetFields: [
      ...
      { key: 'propertySize',     label: 'Size',                         type: 'select', dependsOn: 'propertyType', dependsOnLabel: 'Property Type', optionsMap: PROPERTY_SIZE_BY_TYPE_CLEANING },
  ...
  ac_maintenance: {
    ...
    assetFields: [
      ...
      { key: 'model',             label: 'Model Series',                  type: 'select', dependsOn: 'brand', dependsOnLabel: 'Brand', optionsMap: AC_MODELS_BY_BRAND },
  ...
  pest_control: {
    ...
    assetFields: [
      ...
      { key: 'propertySize',    label: 'Property Size',                  type: 'select', dependsOn: 'propertyType', dependsOnLabel: 'Property Type', optionsMap: PROPERTY_SIZE_BY_TYPE_PEST },
  ...
  real_estate: {
    ...
    assetFields: [
      ...
      { key: 'propertyType',      label: 'Property Type Preferred',       type: 'select', dependsOn: 'requirementType', dependsOnLabel: 'Looking To', optionsMap: PROPERTY_TYPE_BY_REQUIREMENT },
...
// ─── Quick check-in form ──────────────────────────────────────────
// The short subset of each industry's asset fields shown on the public
// check-in form (iPad at the counter / link sent to a caller). Just what
// the business needs to greet and book someone; the full set of fields
// can be filled in later from the dashboard. Order here is display order.
const QUICK_FIELD_KEYS = {
  auto_repair:       ['entityName', 'vehicleType', 'make', 'model', 'year', 'registrationNo'],
  veterinary:        ['entityName', 'petType', 'breed', 'gender', 'dobOrAge'],
  salon_spa:         ['gender', 'preferredStylist', 'preferences'],
  home_cleaning:     ['propertyType', 'propertySize', 'propertyAddress'],
  ac_maintenance:    ['acType', 'brand', 'model', 'capacity', 'locationInProperty'],
  pest_control:      ['propertyType', 'propertySize', 'propertyAddress', 'problemAreas'],
  healthcare_eye:    ['dob', 'gender', 'existingConditions', 'allergies'],
  healthcare_dental: ['dob', 'gender', 'existingConditions', 'allergies'],
  fitness_wellness:  ['dob', 'gender', 'fitnessGoal', 'membershipPlan'],
  real_estate:       ['requirementType', 'propertyType', 'budgetMin', 'budgetMax', 'preferredLocation', 'timeline'],
};

/**
 * Field definitions for the quick check-in form for a vertical.
 * Falls back to the generic config's first fields for unknown verticals.
 */
export function getQuickFields(vertical) {
  const config = getConfig(vertical);
  const keys = QUICK_FIELD_KEYS[vertical];
  if (!keys) return config.assetFields.slice(0, 4);
  return keys.map(k => config.assetFields.find(f => f.key === k)).filter(Boolean);
}
